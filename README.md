...
![image](https://user-images.githubusercontent.com/2777247/206429432-25d1be7c-d394-46b9-abe9-8de01d565fd6.png)
    ...
    2.라이더가 해당 요리를 Pick한 후, 앱을 통해 통보한다.(ok)
    3.고객이 주문상태를 중간중간 조회한다.(ok)
    4.주문상태가 바뀔 때 마다 카톡으로 알림을 보낸다.(ok)
    5.고객이 요리를 배달 받으면 배송확인 버튼을 탭하여, 모든 거래가 완료된다.(ok)
    6.배달 시간이 1시간 이상 초과 시 고객에게 금액 환불 한다.(추가)(ok)
